...
func RotateUUID(cm *panel.ConfigManager, email string) (string, error) {
    ...
    for i := range settings.Clients {
        if settings.Clients[i].Email == email {
            ...
            // Генерируем новый UUID и присваиваем клиенту
            newUUID := uuid.New().String()
            settings.Clients[i].UUID = newUUID
    ...
    // Сериализуем обновлённые клиентские настройки в JSON
    settingsJSON, err := json.Marshal(settings)
    if err != nil {
        return "", fmt.Errorf("ошибка сериализации настроек: %v", err)
    }
    // Обновляем поле settings в объекте inbound
    ...
    if !response.Success {
        return "", fmt.Errorf("ошибка обновления клиента (ID %d): %s", clientID, response.Msg)
    }
    ...
    return "", fmt.Errorf("клиент с ID %d не найден после обновления", clientID)
}
