...
func InitBannedUsersLogger() error {
	logDir := filepath.Dir(BANNED_USERS_LOG_PATH)
	if err := os.MkdirAll(logDir, 0o755); err != nil {
		return err
	}

	logFile, err := os.OpenFile(BANNED_USERS_LOG_PATH, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o666)
	if err != nil {
		return err
	}
...
func LogBannedUser(banInfo *BanInfo) {
	if BannedUsersLogger != nil {
		BannedUsersLogger.Printf("Пользователь: %s, IP адреса: %v, Причина: %s, Забанен до: %s",
			banInfo.Email, banInfo.IPAddresses, banInfo.Reason, banInfo.ExpiresAt.Format("2006-01-02 15:04:05"))
	} else {
